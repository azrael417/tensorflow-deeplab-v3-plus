...
def get_filenames(is_training, data_dir):
  ...
  """
  if is_training:
    files = [x for x in os.listdir(os.path.join(data_dir,'train')) if x.endswith(".h5")]
    _NUM_IMAGES['train'] = len(files)
  else:
    files = [x for x in os.listdir(os.path.join(data_dir,'valid')) if x.endswith(".h5")]
    _NUM_IMAGES['validation'] = len(files)
    
  return files


def parse_record(filename, transpose=True):
  ...
  with h5.File(filename, 'r') as f:
    image = f[image_path][...]
    label = np.expand_dims(f[label_path][...], axis=0)
...
def preprocess_image(image, label, minvals, maxvals, is_training):
  ...
  image = preprocessing.minmax_image_subtraction(image, minvals, maxvals)
  
  return image, label


def input_fn(is_training, data_dir, batch_size, minvals, maxvals, num_epochs=1):
  ...
  Args:
    ...
    minvals: array with minimum pixel values per channel.
    maxvals: array with maximum pixel values per channel.
...
def main(unused_argv):
  ...
  #load the min and max normalization
  normfile = os.path.join(FLAGS.data_dir, "stats.h5")
  if not os.path.isfile(normfile):
    raise ValueError("Please provide a statsfile containing the image normalization per channel")
    
  #open statsfile
  with h5.File(normfile, 'r') as f: #, libver="latest") as f:
    minvals = f['climate']['stats'][:,0].astype(np.float32)
    maxvals = f['climate']['stats'][:,1].astype(np.float32)
  ...
  for _ in range(FLAGS.train_epochs // FLAGS.epochs_per_eval):
    ...
    model.train(
        input_fn=lambda: input_fn(True, FLAGS.data_dir, FLAGS.batch_size, minvals, maxvals, FLAGS.epochs_per_eval),
        hooks=train_hooks,
        # steps=1  # For debug
    )
    ...
    eval_results = model.evaluate(
        ...
        input_fn=lambda: input_fn(False, FLAGS.data_dir, minvals, maxvals, 1),
        hooks=eval_hooks,
        # steps=1  # For debug
    )
    print(eval_results)
